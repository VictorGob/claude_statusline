...
- **Binary size** is ~1.40 MB statically linked on Linux, against 426 KB dynamic (rustc 1.96.1, `x86_64-unknown-linux-gnu`); on Windows it is 339,456 bytes (rustc 1.97.1, `x86_64-pc-windows-gnu`). An empty `println!` program on this profile is 288,904 bytes, so the std floor dominates the dynamic build — and an empty `fn main() {}` on Windows is 252,416 of the 339,456, which is why there is so little size left to trade. Note that size is **not** a free variable here despite the "size is not a constraint" framing above: on Windows it is the mechanism behind the `opt-level` result, so shrinking the image is a speed change. All figures are toolchain-dependent — re-measure rather than trusting them.
